...
is_admin = False
username = "guest"
ghost_mode = False  # Track if a friend is using a custom link without an account
...
# CHECK ROUTE 2: Specific Friend URL Link (?u=alex)
    ...
    ghost_mode = True  # Activates the wipe-on-refresh ghost protocol for safety
...
# --- 4. MASTER SURVEILLANCE SIDEBAR (ONLY FOR ADMIN) ---
if is_admin:
    with st.sidebar:
...
# If an account user wants to log out
if not is_admin and not ghost_mode and st.session_state.logged_in_user is not None:
    with st.sidebar:
...
if "messages" not in st.session_state or ("current_memory_file" in st.session_state and st.session_state.current_memory_file != memory_file):
    if ghost_mode:
        # Ghost users get a clean temporary session state every time the link is clicked fresh
        ...
        # Admin or registered accounts load permanent data from disk file
...
def sync_data_payload(role, content):
    if not ghost_mode or is_admin:
        # Account holders and Admin get a straight write-to-file save
        with open(memory_file, 'w') as file:
            json.dump(st.session_state.messages, file, indent=4)
    else:
        # Ghost URL users get their text cleanly appended to disk file in background
        history = []
        if os.path.exists(memory_file):
            with open(memory_file, 'r') as file:
