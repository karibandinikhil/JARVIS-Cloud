...
# --- 2. THE MASTER PASSTHROUGH CONFIG ---
st.set_page_config(page_title="J.A.R.V.I.S. Network", page_icon="🌐", layout="centered")

# Secret Master Access Key (You can change "ironman123" to whatever passcode you want!)
MASTER_KEY = "ironman123"

with st.sidebar:
    st.header("🔑 Identity Verification")
    access_token = st.text_input("Enter Master Credential:", type="password")
    
    if access_token == MASTER_KEY:
        st.success("Access Granted. Creator Profile Loaded.")
        is_admin = True
        memory_file = "memory_master_sir.json"
    else:
        if access_token:
            st.error("Invalid Credential. Operating in Guest Sandbox Mode.")
        is_admin = False

# --- 3. DYNAMIC MEMORY ROUTING ---
def load_memory():
    # IF YOU ARE ADMIN: Load your permanent file from disk
    if is_admin:
        if os.path.exists(memory_file):
            with open(memory_file, 'r') as file:
                return json.load(file)
        return [{"role": "system", "content": "You are J.A.R.V.I.S., talking to your creator and master. Address him as Sir. Provide deep, maximum-token exhaustive answers."}]
    
    # IF GUEST: Start a fresh, clean, temporary system prompt inside their browser tab
    return [{"role": "system", "content": "You are J.A.R.V.I.S., a helpful, highly advanced AI operating in a cloud environment. Be polite and deeply descriptive, but do not address the user as Sir."}]

# Initialize memory space for the current browser session tab
if "messages" not in st.session_state or ("last_admin_state" in st.session_state and st.session_state.last_admin_state != is_admin):
    st.session_state.messages = load_memory()
    st.session_state.last_admin_state = is_admin

def save_memory_if_admin():
    # Only write to the hard disk if you are the master admin
    if is_admin:
...
if is_admin:
    ...
    st.caption("Master Overrides Active. Long-term memory sync online.")
else:
    st.title("J.A.R.V.I.S. Terminal")
    st.caption("Secure Guest Sandbox Active. Local session isolation engaged.")
